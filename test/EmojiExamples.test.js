const EmojiExamples = artifacts.require("./EmojiExamples")
...
contract('EmojiExamples', ([deployer, user]) => {

    const NAME = 'Emoji Examples'
    const SYMBOL = 'OP'
    const COST = 0
    ...
    describe('Deployment', () => {
        ...
        beforeEach(async () => {
            ...
            emojiExamples = await EmojiExamples.new(
                NAME,
                SYMBOL,
                COST,
                MAX_SUPPLY,
        ...
        it('Returns the contract name', async () => {
            result = await emojiExamples.name()
            result.should.equal(NAME)
        })

        it('Returns the symbol', async () => {
            result = await emojiExamples.symbol()
            result.should.equal(SYMBOL)
        })

        it('Returns the cost to mint', async () => {
            result = await emojiExamples.cost()
            result.toString().should.equal(COST.toString())
        })

        it('Returns the max supply', async () => {
            result = await emojiExamples.maxSupply()
            result.toString().should.equal(MAX_SUPPLY.toString())
        })

        it('Returns the max mint amount', async () => {
            result = await emojiExamples.maxMintAmount()
            result.toString().should.equal('1')
        })

        it('Returns the time deployed', async () => {
            result = await emojiExamples.timeDeployed()
        ...
        it('Returns the amount of seconds from deployment to wait until minting', async () => {
            ...
            result = await emojiExamples.allowMintingAfter()
        ...
        it('Returns how many seconds left until minting allowed', async () => {
            let buffer = 2
            let target = Number(milliseconds.toString().slice(0, 3))
            result = await emojiExamples.getSecondsUntilMinting()
            result = Number(result)
        ...
        it('Returns current pause state', async () => {
            result = await emojiExamples.isPaused()
            result.toString().should.equal('false')
        })

        it('Returns current reveal state', async () => {
            result = await emojiExamples.isRevealed()
    ...
    describe('Minting', async () => {
        describe('Success', async () => {
            ...
            beforeEach(async () => {
                ...
                result = await emojiExamples.mint(1, { from: user, value: web3.utils.toWei('0', 'ether') })
            })
            ...
            it('Updates the total supply', async () => {
                result = await emojiExamples.totalSupply()
                result.toString().should.equal('1')
            })

            it('Returns IPFS URI', async () => {
                result = await emojiExamples.tokenURI(1)
                result.should.equal(`${IPFS_IMAGE_METADATA_URI}1.json`)
            })

            it('Returns how many a minter owns', async () => {
                result = await emojiExamples.balanceOf(user)
                result.toString().should.equal('1')
            })

            it('Returns the IDs of minted NFTs', async () => {
                result = await emojiExamples.walletOfOwner(user)
        ...
        describe('Failure', async () => {
            ...
            it('Attempt to mint before mint date', async () => {
                await emojiExamples.mint(1, { from: user, value: web3.utils.toWei('0', 'ether') }).should.be.rejectedWith(EVM_REVERT)
            })
        })
    })
    ...
    describe('Updating Contract State', async () => {
        describe('Success', async () => {
            ...
            it('Sets the cost', async () => {
                ...
                await emojiExamples.setCost(cost, { from: deployer })
                result = await emojiExamples.cost()
            ...
            it('Sets the pause state', async () => {
                ...
                await emojiExamples.setIsPaused(isPaused, { from: deployer })
                result = await emojiExamples.isPaused()
            ...
            it('Sets the reveal state', async () => {
                ...
                await emojiExamples.setIsRevealed(isRevealed, { from: deployer })
                result = await emojiExamples.isRevealed()
            ...
            it('Sets the max batch mint amount', async () => {
                ...
                await emojiExamples.setmaxMintAmount(5, { from: deployer })
                result = await emojiExamples.maxMintAmount()
            ...
            it('Sets the IPFS not revealed URI', async () => {
                ...
                await emojiExamples.setNotRevealedURI(uri, { from: deployer })
                result = await emojiExamples.notRevealedUri()
            ...
            it('Sets the base extension', async () => {
                ...
                await emojiExamples.setBaseExtension('.example', { from: deployer })
                result = await emojiExamples.baseExtension()
