...
// Import ABI + Config
import EmojiExamples from '../abis/EmojiExamples.json'
...
function App() {
	...
	const [emojiExamples, setEmojiExamples] = useState(null)
	...
	const loadBlockchainData = async (_web3, _account, _networkId) => {
		...
		try {
			const emojiExamples = new _web3.eth.Contract(EmojiExamples.abi, EmojiExamples.networks[_networkId].address)
			setEmojiExamples(emojiExamples)

			const maxSupply = await emojiExamples.methods.maxSupply().call()
			const totalSupply = await emojiExamples.methods.totalSupply().call()
			setSupplyAvailable(maxSupply - totalSupply)

			const allowMintingAfter = await emojiExamples.methods.allowMintingAfter().call()
			const timeDeployed = await emojiExamples.methods.timeDeployed().call()
			setRevealTime((Number(timeDeployed) + Number(allowMintingAfter)).toString() + '000')

			if (_account) {
				const ownerOf = await emojiExamples.methods.walletOfOwner(_account).call()
				setOwnerOf(ownerOf)
				console.log(ownerOf)
			} else {
				setOwnerOf([])
	...
	const mintNFTHandler = async () => {
		...
		// Mint NFT
		if (emojiExamples && account) {
			setIsMinting(true)
			setIsError(false)

			await emojiExamples.methods.mint(1).send({ from: account, value: 1 })
				.on('confirmation', async () => {
					const maxSupply = await emojiExamples.methods.maxSupply().call()
					const totalSupply = await emojiExamples.methods.totalSupply().call()
					setSupplyAvailable(maxSupply - totalSupply)

					const ownerOf = await emojiExamples.methods.walletOfOwner(account).call()
					setOwnerOf(ownerOf)
				})
				.on('error', (error) => {
					window.alert(error)
				...
				<section id='about' className='about'>
							   ...
							   <CrossmintPayButton
							  	...
							  	/>
									...
									{ownerOf.length > 0 &&
										<p><small>
											<a
												href={`${openseaURL}/assets/${emojiExamples._address}/${ownerOf[0]}`}
												target='_blank'
					...
					<Row className='m-3' style={{ marginTop: "16vh", marginBottom: "4vh", paddingBottom: "4vh" }}>
						<Col>
							{emojiExamples &&
								<a
									href={`${explorerURL}/address/${emojiExamples._address}`}
									target='_blank'
									className='text-center'>
									Contract: {emojiExamples._address}
								</a>
							}
						</Col>
					</Row>
